...
int diffusion_volumes_problem_init(void) {
  ...
  milonga_allocate_global_matrices(milonga.spatial_unknowns * milonga.groups,
                                   wasora_mesh.main_mesh->max_faces_per_element + milonga.groups,
                                   milonga.groups);
...
int diffusion_volumes_normalize_flux(void) {
  ...
  if (wasora_var(milonga.vars.power) == 0) {
    ...
    for (i = 0; i < wasora_mesh.main_mesh->n_cells; i++) {
      ...
      for (g = 0; g < milonga.groups; g++) {
        VecGetValues(milonga.phi, 1, &wasora_mesh.main_mesh->cell[i].index[g], &milonga.functions.phi[g]->data_value[i]);
    ...
    for (i = 0; i < wasora_mesh.main_mesh->n_cells; i++) {
      if (wasora_mesh.main_mesh->cell[i].element->physical_entity != NULL &&
        ...
        for (g = 0; g < milonga.groups; g++) {
          VecGetValues(milonga.phi, 1, &wasora_mesh.main_mesh->cell[i].index[g], &milonga.functions.phi[g]->data_value[i]);
          den += diffusion_volumes_cell_integral(&wasora_mesh.main_mesh->cell[i], xs->eSigmaF[g]) * milonga.functions.phi[g]->data_value[i];
        }
      }
    }
